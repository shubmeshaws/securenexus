...
export type NodeChangeDirection = 'all' | 'increase' | 'decrease';
...
export interface NodeChangesQuery extends DashboardDateQuery {
  cluster?: string;
  date?: string;
  direction?: NodeChangeDirection;
}

export interface NodeChangesResponse {
  cluster: string;
  calendarDate: string;
...
let changesCache: { key: string; at: number; data: NodeChangesResponse } | null = null;

function cacheKey(query: NodeChangesQuery): string {
...
/** Hours to include for a calendar day in IST. */
...
function bucketSamplesByIstHour(
  samples: Array<{ nodeCount: number; sampledAt: Date }>,
  calendarDate: string,
  allowedHours: number[]
): Map<number, { nodeCount: number; sampledAt: Date }> {
  const allowed = new Set(allowedHours);
  const byHour = new Map<number, { nodeCount: number; sampledAt: Date }>();
...
function buildSampleRows(
  calendarDate: string,
  byHour: Map<number, { nodeCount: number; sampledAt: Date }>,
  allowedHours: number[],
  ...
  for (const hour of allowedHours) {
    ...
    const delta = previous != null ? sample.nodeCount - previous : null;
...
async function getPreviousBaseline(
  ...
  const sample = await prisma.clusterNodeHourlySample.findFirst({
    where: {
      clusterName,
      sampledAt: {
...
export async function getNodeChanges(query: NodeChangesQuery = { days: 14 }): Promise<NodeChangesResponse> {
  const direction: NodeChangeDirection =
  ...
  const empty: NodeChangesResponse = {
    cluster: selectedCluster,
  ...
  const data: NodeChangesResponse = {
    cluster: selectedCluster,
    calendarDate,
    availableClusters,
    rows,
...
export function invalidateNodeChangesCache(): void {
  changesCache = null;
}

export function parseNodeChangesQuery(query: {
  days?: string | string[];
  from?: string | string[];
  to?: string | string[];
