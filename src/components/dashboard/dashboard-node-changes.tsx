...
import type { NodeChangeDirection, NodeChangesResponse } from '@/lib/node-changes-service';
import { cn, formatTime12h } from '@/lib/utils';

export function DashboardNodeChanges({
  dateRange,
  onDateRangeChange,
}: {
  ...
  const [directionFilter, setDirectionFilter] = useState<NodeChangeDirection>('all');
  const [calendarDate, setCalendarDate] = useState('');
  const rangeReady = isDashboardDateRangeReady(dateRange);
  const periodLabel = getDashboardPeriodLabel(dateRange);

  const queryUrl = useMemo(() => {
    const base = appendDashboardDateQuery('/api/dashboard/node-changes', dateRange);
  ...
  const { data, isLoading, isFetching } = useQuery({
    queryKey: ['node-changes', dateRange, clusterFilter, directionFilter, calendarDate],
    queryFn: () => apiFetch<NodeChangesResponse>(queryUrl),
    enabled: rangeReady,
    staleTime: 60_000,
      ...
      <GlassPanel className="flex flex-col">
          ...
          action={
            <DashboardFilterBar className="justify-end">
              ...
              <DashboardFilterSelect
                ...
                onChange={(e) => setDirectionFilter(e.target.value as NodeChangeDirection)}
        ...
        {!rangeReady ? (
          <p className="p-8 text-center text-sm text-muted-foreground">
            Select a from and to date to load node changes.
          </p>
        ) : isLoading && !data ? (
          ...
          <p className="p-8 text-center text-sm text-muted-foreground">
            Add EKS clusters under Clusters to start tracking node count changes.
          </p>
        ) : !rows.length ? (
          <p className="p-8 text-center text-sm text-muted-foreground">
            No node count samples for this day
