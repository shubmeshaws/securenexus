...
function jobForResource(id: string) {
  ...
  if (job.resourceId !== id || job.action !== 'clone') {
    return {
      running: false,
      resourceId: null,
      action: null,
...
async function postHandler(req: AuthenticatedRequest, res: NextApiResponse) {
  ...
  if (current.running) {
    if (current.resourceId === id && current.action === 'clone') {
      setNoCacheHeaders(res);
  ...
  const started = startSecurityResourceSyncJob(id, 'clone');
  if (!started) {
    return res.status(409).json({ error: 'Clone is already running.' });
  }
