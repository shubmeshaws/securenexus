...
- [Performance & EC2 sizing](#performance--ec2-sizing)
...
| **Instance type** | `t3a.large` (2 vCPU, 8 GB) | `t3a.xlarge` (4 vCPU, 16 GB) or app on `t3a.large` + **RDS** |
| **Memory** | 8 GB RAM (app + Postgres on same host) | 16 GB RAM, or 8 GB app host + separate DB |
| **EBS volume** | 20 GB `gp3` | 30 GB `gp3` |
| **Network** | Elastic IP (optional but recommended) | Elastic IP attached |

> **Note:** `t3a.medium` (4 GB) works for dev or very light use (&lt;10 schedules). For production with ~20 concurrent schedules, UI polling, and Postgres on the same box, use **at least `t3a.large`**. See [Performance & EC2 sizing](#performance--ec2-sizing).
...
## Performance & EC2 sizing

SecureNexus is **I/O-bound** (Kubernetes, ArgoCD, AWS APIs) with **memory spikes** during schedule bursts and dashboard loads. On a small instance, **RAM pressure** (app + Postgres sharing 4 GB) often causes slowness before raw CPU limits.

### CPU vs RAM — what the app uses

| Resource | Typical profile |
|----------|-----------------|
| **CPU** | Moderate bursts during schedule ticks and large K8s/Argo JSON responses. Node.js runs on **one JS thread**; parallel work is async I/O, not one thread per vCPU. |
| **RAM** | Main constraint on small EC2. Node (~0.5–1.5 GB) + Postgres (~0.5–1 GB) + OS headroom. Full-cluster K8s lists and dashboard analytics can spike heap usage. |

The scheduler does **not** automatically saturate every vCPU core. Parallelism is controlled by env vars (see below). More RAM and a separate database usually help more than adding cores alone.

### How schedule execution works

- Cron runs **every minute** and finds schedules whose shutdown/startup time matches that minute.
- Each schedule is **claimed once per minute** in the database (no duplicate runs for the same schedule in the same minute).
- Up to **`SCHEDULE_CONCURRENCY`** schedules run K8s/Argo work **at the same time** (global pool across overlapping minutes).
- **Overlapping minutes are supported** — if a 9:00 PM batch is still running, schedules due at 9:02 PM still execute (they are not skipped).
- Namespace schedules scale workloads with up to **`WORKLOAD_OP_CONCURRENCY`** parallel K8s ops per schedule.

**Example — 20 schedules at the same time (default `SCHEDULE_CONCURRENCY=8`):**

| Avg time per schedule | Approx. total time |
|----------------------|-------------------|
| ~30 seconds | ~2 minutes |
| ~60 seconds | ~4 minutes |

Startup actions missed during a stopped window can **catch up within 2 hours**. Shutdown uses the exact scheduled minute — stagger shutdown times by a few minutes if you mix large batches with schedules 1–3 minutes apart.

### Recommended instance types

**All-in-one (app + Postgres on same EC2):**

| Your workload | Instance | Notes |
|---------------|----------|--------|
| Dev / &lt;10 schedules | `t3a.medium` | Tight; OK for testing |
| Production ~20 schedules | **`t3a.large`** | **Minimum** recommended |
| Comfortable production | **`t3a.xlarge`** | Best single-box option |
| Heavy / many clusters | `m6a.large` or `m7a.large` | Non-burstable CPU for steady schedule storms |

**Best production setup:** **`t3a.large`** (app) + **RDS `db.t4g.small`** (database) — frees RAM on the app host.

**Burstable `t3a` note:** Sustained schedule + K8s load can consume CPU credits. For predictable execution at peak times, prefer **`m6a`/`m7a`** (fixed performance) or **`t3a.xlarge`**.

### Tuning environment variables

Add to `.env` on EC2 (restart the app after changes):

```env
# Parallel schedule executions (default 8 ≈ 2 vCPU). Min 4, max 32.
SCHEDULE_CONCURRENCY=8

# Parallel K8s scale ops per namespace schedule (default matches above).
WORKLOAD_OP_CONCURRENCY=8

# Cap Node heap so Postgres keeps RAM (adjust for instance size)
NODE_OPTIONS=--max-old-space-size=3072
```

| Instance | Suggested `SCHEDULE_CONCURRENCY` | Suggested `NODE_OPTIONS` |
|----------|----------------------------------|---------------------------|
| `t3a.medium` (4 GB) | `6` | `--max-old-space-size=2048` |
| **`t3a.large` (8 GB)** | **`8`** (default) | `--max-old-space-size=3072` |
| `t3a.xlarge` (16 GB) | `16` | `--max-old-space-size=6144` |

On startup, logs show: `[PodScheduler] Initializing schedule runner (every minute, concurrency=8)...`

### Built-in performance optimizations

- **Global schedule execution pool** — caps concurrent schedule runs across overlapping cron ticks
- **ArgoCD app list cache** (90s) — shared across overview, schedules, and scheduler actions
- **Kubeconfig cache** (10 min per cluster) — fewer EKS token / config rebuilds
- **Server-side API caches** — overview, dashboard insights, workload scans
- **UI polling** — 120s intervals, no background refetch when the tab is hidden
- **Parallel namespace workload ops** — scale multiple deployments in a namespace concurrently
...
Always **restart** after `git pull` or `.env` changes so Prisma client and concurrency settings reload.
...
# Optional — schedule parallelism (defaults suit ~2 vCPU / t3a.large)
# SCHEDULE_CONCURRENCY=8
# WORKLOAD_OP_CONCURRENCY=8
# NODE_OPTIONS=--max-old-space-size=3072
```
...
For production sizing and parallelism, see [Performance & EC2 sizing](#performance--ec2-sizing). Example PM2 env in `ecosystem.config.cjs` or `.env`:
...
| `SCHEDULE_CONCURRENCY` | No | Max schedules running K8s/Argo at once (default **8**, min 4, max 32). See [Performance & EC2 sizing](#performance--ec2-sizing). |
| `WORKLOAD_OP_CONCURRENCY` | No | Parallel scale ops per namespace schedule (default matches `SCHEDULE_CONCURRENCY`) |
| `NODE_OPTIONS` | No | Node.js heap limit, e.g. `--max-old-space-size=3072` on 8 GB instances |
...
│   │   ├── scheduler-runner.ts
│   │   ├── schedule-execution-pool.ts  # Global schedule concurrency pool
│   │   ├── k8s-client.ts
│   │   ├── argocd-client.ts
│   │   └── ...
